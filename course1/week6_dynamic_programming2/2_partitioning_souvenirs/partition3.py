...
def findItemsWithCap(capacity, items):
    ...
    for itemIndex in range(1,n):
        ...
        for capIndex in range(1,min(items[itemIndex],capacity)):
        ...
        for capIndex in range(items[itemIndex],capacity+1):
            ...
            if dynamicMat[capIndex][itemIndex]==capacity:
                return backTrackSol(dynamicMat,items,(capIndex,itemIndex))
    #print(dynamicMat)
    return None
...
def partition3update(A):
    target = sum(A)//3
    #print("A[1:]=%s"%str(A[1:]))
    FirstBucket=findItemsWithCap(target-A[0],A[1:])
    #print(FirstBucket)
    if FirstBucket==None:
        return 0
    else:
        RemaingItems=[]
        for idx in range(len(FirstBucket)):
            if not FirstBucket[idx]:
                RemaingItems.append(A[idx+1])
        #print("RemaingItems:%s"%str(RemaingItems))
        secondBucket=findItemsWithCap(target-RemaingItems[0],RemaingItems[1:])
        if secondBucket==None:
            return 0
        else: return 1
